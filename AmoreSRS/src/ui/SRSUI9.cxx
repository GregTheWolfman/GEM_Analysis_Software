...
namespace amore {
  namespace SRS {
	...
	delete fMapping ;
      ...
      //===========================================================================================================
      void SRSUI9::Construct() { // The custom GUI is constructed here. gRootFrame is the container of the custom widgets.
	printf("SRSUI9::Construct() = > let Start to display with %s run\n", fRunType.Data()) ;

	//=== Class in which TGTab are created 
	fTab = new TGTab(amore::ui::gRootFrame);
	amore::ui::gRootFrame->AddFrame(fTab);
	TQObject::Connect(fTab, "Selected(Int_t)", "amore::SRS::ui::SRSUI9", this, "SlotSelectedTab(Int_t)");
	...
	if(fRunType == "RAWDATA") {
	  ...
	  for (rowMap_it = fRowMap.begin(); rowMap_it != fRowMap.end(); ++rowMap_it) {
	    ...
	    if (canvasName == "RAWDATA_ALLAPVs") {
	      ...
	      printf("SRSUI9::Construct() = > canvas nbOfAPVs = %d, nbCols = %d, nbRows = %d length = %d width = %d\n",nbOfAPVs , nbOfCols, nbOfRows, length, width) ;
		...
		TString canvasname = "gui9_rawdata" + canvasId ;
		...
		printf("SRSUI9::Construct() = > canvas No %d, has %d cols and %d rows and %d length and %d width\n", i, nbOfCols, nbOfRows, length, width) ;
	      }
	    }
	  ...
	  for (rowMap_it = fRowMap.begin(); rowMap_it != fRowMap.end(); ++rowMap_it) {
	    ...
	    Int_t length = fLengthMap[canvasName] ;
	    Int_t width  = fWidthMap[canvasName] ;
	    TString canvasname = "gui9_"+ canvasName ;
	    tempFrame = fTab->AddTab(canvasname) ;	  
	    fEC[icanvas] = new TRootEmbeddedCanvas(canvasname, tempFrame, length, width) ;
	    tempFrame->AddFrame(fEC[icanvas]) ;
	...
	printf("SRSUI9::Construct() = > Exit Construct \n") ;
      }

      //===========================================================================================================
      void SRSUI9::SubscribeMonitorObjects() { // This is now deprecated, not recognized by the framework
      }

      //===========================================================================================================
      void SRSUI9::SlotSelectedTab(Int_t id) {
	switch(id) {
	case 0:
	  MySubscribe();
	  fSelectedTab = 0;
	...
	if(fRunType == "RAWDATA") {
      ...
      //===========================================================================================================
      void SRSUI9::ClearDisplayParameters() {
	fRowMap.clear();
	fColMap.clear();
	fWidthMap.clear();
	fLengthMap.clear();
	fHistoNameMap.clear();
      ...
      //===========================================================================================================
      Bool_t SRSUI9::SetDisplayParameters() {
	ClearDisplayParameters() ;
	...
	const char * displayCfg = fSRSConf->GetDisplayCfgName() ;
        printf("SRSUI9::SetDisplayParameters()->Loading Display from  %s \n", displayCfg) ;

	ifstream filestream (gSystem->ExpandPathName(displayCfg), ifstream::in) ;
        if(!filestream.is_open()) {
          printf("SRSUI9::SetDisplayParameters() ==> CAN'T OPEN THE FILE  \n")  ;
          return kFALSE ;
        }
	...
	while (line.ReadLine(filestream)) {
	  ...
	  while (TObjString * st = (TObjString*) myiter.Next()) {
	    ...
	    if (s == "CANVAS") {
	      ...
	      //	      printf("SRSUI9::SetDisplayParameters() ==> Canvas=%s, nbOfCols=%d, nbOfRows=%d, canvasLength=%d, canvasWidth=%d \n",canvasName.Data(), col, row, length, width )  ;
	    }
	...
	return kTRUE ;
      ...
      //===========================================================================================================
      void SRSUI9::RootDisplaySetup(Int_t ncount) { 
	gROOT->Reset() ;
	gStyle->Reset() ;
	...
	gStyle->SetNumberContours(NCont);
      ...
      //===========================================================================================================
      void SRSUI9::StartOfCycle() {
      }

      //===========================================================================================================
      void SRSUI9::EndOfCycle() {
      }

      //===========================================================================================================
      bool SRSUI9::StartAuto()  {
