...
namespace amore {
  namespace SRS {
	...
	delete fMapping ;
      ...
      //===========================================================================================================
      void SRSUI4::Construct() { // The custom GUI is constructed here. gRootFrame is the container of the custom widgets.
	printf("SRSUI4::Construct() = > let Start to display with %s run\n", fRunType.Data()) ;

	//=== Class in which TGTab are created 
	fTab = new TGTab(amore::ui::gRootFrame);
	amore::ui::gRootFrame->AddFrame(fTab);
	TQObject::Connect(fTab, "Selected(Int_t)", "amore::SRS::ui::SRSUI4", this, "SlotSelectedTab(Int_t)");
	...
	if(fRunType == "RAWDATA") {
	  ...
	  for (rowMap_it = fRowMap.begin(); rowMap_it != fRowMap.end(); ++rowMap_it) {
	    ...
	    if (canvasName == "RAWDATA_ALLAPVs") {
	      ...
	      printf("SRSUI4::Construct() = > canvas nbOfAPVs = %d, nbCols = %d, nbRows = %d length = %d width = %d\n",nbOfAPVs , nbOfCols, nbOfRows, length, width) ;
		...
		TString canvasname = "gui4_rawdata" + canvasId ;
		...
		printf("SRSUI4::Construct() = > canvas No %d, has %d cols and %d rows and %d length and %d width\n", i, nbOfCols, nbOfRows, length, width) ;
	      }
	    }
		...
		*/
		...
		TString canvasname =  "gui4_"+ name.Remove(0,8);
		tempFrame = fTab->AddTab(canvasname) ;
		fEC[icanvas] = new TRootEmbeddedCanvas(canvasname, tempFrame, length, width) ;
		tempFrame->AddFrame(fEC[icanvas]) ;
		printf("SRSUI4::Construct() = > canvas %s with canvasID %d, has %d cols and %d rows and %d length and %d width\n", canvasName.Data(), icanvas, nbOfCols, nbOfRows, length, width) ;
		icanvas++ ;
	      }
	    }
	  ...
	  for (rowMap_it = fRowMap.begin(); rowMap_it != fRowMap.end(); ++rowMap_it) {
	    ...
	    Int_t length = fLengthMap[canvasName] ;
	    Int_t width  = fWidthMap[canvasName] ;
	    TString canvasname = "gui4_"+  canvasName;
	    tempFrame = fTab->AddTab(canvasname) ;	  
	    fEC[icanvas] = new TRootEmbeddedCanvas(canvasname, tempFrame, length, width) ;
	    tempFrame->AddFrame(fEC[icanvas]) ;
	    fEC[icanvas]->GetCanvas()->Divide(nbOfRows, nbOfCols) ;
	...
	printf("SRSUI4::Construct() = > Exit Construct \n") ;
      }

      //===========================================================================================================
      void SRSUI4::SubscribeMonitorObjects() { // This is now deprecated, not recognized by the framework
      }

      //===========================================================================================================
      void SRSUI4::SlotSelectedTab(Int_t id) {
	switch(id) {
	case 0:
	  MySubscribe();
	  fSelectedTab = 0;
	...
	if(fRunType == "RAWDATA") {
      ...
      //===========================================================================================================
      void SRSUI4::ClearDisplayParameters() {
	fRowMap.clear();
	fColMap.clear();
	fWidthMap.clear();
	fLengthMap.clear();
	fHistoNameMap.clear();
      ...
      //===========================================================================================================
      Bool_t SRSUI4::SetDisplayParameters() {
	ClearDisplayParameters() ;
	...
	const char * displayCfg = fSRSConf->GetDisplayCfgName() ;
        printf("SRSUI4::SetDisplayParameters()->Loading Display from  %s \n", displayCfg) ;

	ifstream filestream (gSystem->ExpandPathName(displayCfg), ifstream::in) ;
        if(!filestream.is_open()) {
          printf("SRSUI4::SetDisplayParameters() ==> CAN'T OPEN THE FILE  \n")  ;
          return kFALSE ;
        }
	...
	while (line.ReadLine(filestream)) {
	  ...
	  while (TObjString * st = (TObjString*) myiter.Next()) {
	    ...
	    if (s == "CANVAS") {
	      ...
	      //	      printf("SRSUI4::SetDisplayParameters() ==> Canvas=%s, nbOfCols=%d, nbOfRows=%d, canvasLength=%d, canvasWidth=%d \n",canvasName.Data(), col, row, length, width )  ;
	    }
	...
	return kTRUE ;
      ...
      //===========================================================================================================
      void SRSUI4::RootDisplaySetup(Int_t ncount) { 
	gROOT->Reset() ;
	gStyle->Reset() ;
	...
	gStyle->SetNumberContours(NCont);
      ...
      //===========================================================================================================
      void SRSUI4::StartOfCycle() {
      }

      //===========================================================================================================
      void SRSUI4::EndOfCycle() {
      }

      //===========================================================================================================
      bool SRSUI4::StartAuto()  {
